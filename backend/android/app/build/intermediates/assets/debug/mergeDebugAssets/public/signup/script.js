	...
	try {
		const response = await fetch(`${window.BUSTRAVEL_API || 'http://localhost:3000'}/api/auth/signup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fullName: fields[0].value.trim(), mobile: fields[1].value.trim(), email: fields[2].value.trim(), password, confirmPassword }) });
