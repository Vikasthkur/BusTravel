...
document.querySelector('#searchForm').addEventListener('submit', async (event) => {
	...
	list.innerHTML = '';
	try {
		const response = await fetch(`${window.BUSTRAVEL_API || ''}/api/buses?from=${encodeURIComponent(fields[0].value.trim())}&to=${encodeURIComponent(fields[1].value.trim())}`);
		const result = await response.json();
		if (!response.ok) throw new Error(result.error || 'Could not search buses.');
		...
		document.querySelector('#message').textContent = error.message || 'Cannot connect to server.';
	}
});